...
FROM (
    SELECT 
        ...
        RANK() OVER (
            PARTITION BY ID_WYPOZYCZALNIA_SKUTER
            ORDER BY COUNT(*) DESC
        ) AS RANKING
    FROM H_WYPOZYCZENIA
...
ORDER BY wyp.NAZWA, agg.RANKING;
...
FROM (
    SELECT 
        ...
        DENSE_RANK() OVER (ORDER BY AVG(w.POJEMNOSC) DESC) AS RANKING
...
FROM (
    SELECT 
        ...
        DENSE_RANK() OVER (PARTITION BY f.ID_WYPOZYCZALNIA_PRACOWNIK ORDER BY COUNT(*) DESC) AS RANKING
