...
INSERT INTO H_PANSTWO (ID, NAZWA) SELECT ID, NAZWA FROM PANSTWO;
...
INSERT INTO H_PRODUCENT (ID, NAZWA) SELECT ID, NAZWA FROM PRODUCENT;
INSERT INTO H_MODEL (ID, NAZWA) SELECT id, nazwa FROM MODEL;
INSERT INTO h_kolor (ID, nazwa) SELECT ID, NAZWA FROM kolor;
INSERT INTO H_TYP_NAPEDU (ID, NAZWA) SELECT ID, NAZWA FROM TYP_NAPEDU;
INSERT INTO H_PAKIET_WYPOSAZENIA (ID, NAZWA) SELECT ID, NAZWA FROM PAKIET_WYPOSAZENIA;
...
INSERT INTO H_WYPOZYCZENIA (
    ...
    POJEMNOSC, ROK_PRODUKCJI,
    ID_WYPOZYCZALNIA_SKUTER, ID_PANSTWO_SKUTER, ID_WOJEWODZTWO_SKUTER, ID_MIASTO_SKUTER,
    ID_PRACOWNIK, ID_WYPOZYCZALNIA_PRACOWNIK, ID_PANSTWO_PRACOWNIK, ID_WOJEWODZTWO_PRACOWNIK, ID_MIASTO_PRACOWNIK,
    CENA,
...
JOIN WYPOZYCZALNIA wyp ON p.ID_WYPOZYCZALNIA = wyp.ID
JOIN ULICA up ON wyp.ID_ULICA = up.ID
JOIN MIASTO mp ON up.ID_MIASTO = mp.ID
JOIN WOJEWODZTWO wp ON mp.ID_WOJEWODZTWO = wp.ID
JOIN PANSTWO pp ON wp.ID_PANSTWO = pp.ID;
